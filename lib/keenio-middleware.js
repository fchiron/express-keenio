...
KeenioMiddleware.prototype._finishResponse = function (req, _parsedResponseData, routeConfig) {
  ...
  return function onResponseFinish() {
    ...
    try {
      ...
      // Check if this event should be tracked (by default, all events are tracked)
      if (!self.shouldTrackEventHandler(keenEvent, parsedRequestData, parsedResponseData)) {
        return false;
      }
...
KeenioMiddleware.prototype._configureHandlers = function (options) {
  ...
  this.shouldTrackEventHandler = options.handlers.shouldTrackEvent || function () { return true; }; // Track all events by default
};
